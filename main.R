library(MASS)
datad = read.csv("E:/U/new_data14-16.csv",header = FALSE) 
time = 10 #Number of repetitions
count = 5 #Stock dimension: IP
selected = read.csv("E:/U/selected_5.csv",header = FALSE) #"Randomly selected stocks"
selected= as.matrix(selected)

for(t1 in 1: time){
  data = datad[,selected[t1,]]
  ...
  for(a in 1:length(w)){
    ...
    for ( i in 1:(n-t-1)){
      data1 = data[(i+1):(i+t),] 
      data2 = data[(i):(i+t-1),]
      mat_t_ni_3 = mat_t_solve(data1,data2)
      if(!is.na(mat_t_ni_3)[1,1,1]){
        w_mat = w_solve(mat_t_ni_3)
  ...
  write.table(jieguo,file=paste0("E:/U/result/",count ,"IP",t1,".csv"),sep=",",row.names=TRUE,col.names=TRUE,quote=TRUE)
}
